...
convert_bayesian_output_to_df <- function(output_string, column_names) {
  ...
  for (line in lines) {
    ...
    if (grepl("^\\s*[0-9.]+\\s*[0-9.]+", line)) {
      ...
      temp_df <- data.frame(Node = current_node,
...
#   # Define the wait_message reactive expression
#   wait_message <- reactive({
#     if (!server_ready()) {
#       return(tags$div(
#         id = "wait-message",
#         style = "position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);",
#         "Please wait while the server is still initializing..."
#       ))
#     }
#   })

  # Render the wait message UI output
  # output$wait_message <- renderUI({
  #   wait_message()
  # })

  # # Set server_ready to TRUE when the server is ready
  # observe({
  ...
  observeEvent(input$twoCARTgo, {
    if(!is.null(input$twoCARTVar1)) {
      ...
      try({
        ...
        #ltc_numeric <- lapply(ltc, as.numeric)

          # create a new data frame with the converted values
          #data <- data.frame(lapply(Query[, c(unique(input$twoCARTVar1), input$twoCARTVar2)], 
          #                                function(x) ifelse(names(x) %in% names(ltc_numeric), 
          #                                                    ltc_numeric[[which(names(ltc_numeric) == names(x))]], 
          #                                                    x)))
...
#}
    ...
    #rules_df <- extract_rules(model)
    
    ## Rpart rules
    
    
    #rules_df <- rules_df[, c("id", "combined")]
  ...
  #output$saved_rules <- renderText({
    node <-read_html(gsub('^.*Rules\\s*|\\s*\\$.*$', '', input$clicked_nodeId)) %>% html_text#strip_html(input$clicked_nodeId))
    #pattern <- "(\\d+\\.?\\d*)\\s*(>=|>)\\s*(\\w+)\\s*(<|=)\\s*(\\d+\\.?\\d*)"
    #matches <-regmatches(node,regexec(pattern,node))
    ...
    filter_obj <- convert_to_json(node,colnames(data))
     ...
     append_to_col_lookup <- function(col_lookup, new_columns) {
      ...
      col_lookup <- c("Age"  = "AgeDimension", "Sex" = "SexDimension",                                                            
 "Risk Score Int" = "RskDimension", "mosaic label" = "MDimension",                                          
 "deprivation decile" = "DDimension", "household type" = "ADimension" ,                                                            
 "electoral ward or division" = "WDimension",
 "Asthma" = "Asthma"                                   , "Coronary Artery Disease" = "Coronary Artery Disease",                 
 "Congestive Heart Failure" = "Congestive Heart Failure"                 , "Cancer" = "Cancer",                                  
 "Chronic obstructive pulmonary disease" = "COPD"   , "Persistent depressive disorder" = "Depression" ,         
 "Diabetes"   = "Diabetes"                              , "Hypertension" = "Hypertension",                            
 "Atrial fibrillation" = "Atrial Fibrillation"                      , "Chronic kidney disease" = "Chronic Kidney Disease",                  
 "Dementia"   = "Dementia"                              , "Epilepsy" = "Epilepsy" ,                               
 "Hypothyroid"     = "Hypothyroid"                         , "Mental health" = "Mental Health",                           
 "Learning disability" = "Learning Disabilities"                     , "Osteoporosis" = "Osteoporosis",                            
 "Peripheral artery disease" = "Peripheral Artery Disease"               , "Rheumatoid arthritis" = "Rheumatoid Arthritis",
 "Risk Score" = "RskDimension", "Risk Score Group" = "RskDimension",
 "top 20 percent deprived" = "DDimension",
 "age band narrow" = "AgeDimension", "age band broad" = "AgeDimension", "age markers" = "AgeDimension",
 "age 55 and over" = "AgeDimension", "age 65 and over" = "AgeDimension", "age 75 and over" = "AgeDimension", "age 17-54" = "AgeDimension", "age Children" = "AgeDimension"
...
#### for deriving columns
 ...
 if ("Risk Score Group" %in% names(fromJSON(filter_obj))) {
  ...
  #new_json_string <- toJSON(json_list)
...
# Parse the initial JSON string into a list
      ...
      if ("age markers" %in% names(fromJSON(filter_obj))) {
       ...
       if ("age 17-54" %in% names(fromJSON(filter_obj))) {
        ...
        ###NEED TO MAKE FOR WHEN THERE ISNT A LTC E.G HYPERTENSION = 0

        ## all others other than =0 need to be selected.
      ...
      # # Convert the "groups" values and replace them in the list
      # json_list$`age markers` <- string_to_groups_age(paste(unlist(json_list$`age markers`), collapse = " "))
      ...
      ##convert some columns e.g groups to score ranges
...
# Group columns
group_cols <- c("Asthma", "Coronary Artery Disease", "Congestive Heart Failure", "Cancer", "COPD", "Depression", "Diabetes", "Hypertension", "Atrial Fibrillation", "Chronic Kidney Disease",
"Dementia", "Epilepsy", "Hypothyroid", "Mental Health", "Learning Disabilities", "Osteoporosis", "Peripheral Artery Disease", "Rheumatoid Arthritis")
...
modify_json <- function(json_str, group_cols) {
  ...
  # group_col_values_0 <- sapply(unlist(json_list[group_cols]), function(x) x == 0)
  # print(group_col_values_0)



    
  #print(group_col_values_0 = group_col_values)

  # Extract the names of the group columns that have a value of 1
  selected_group_cols <-names(which(unlist(group_col_values)))


  #  selected_group_cols <- group_cols[group_col_values]
  
  # Combine the selected group columns into a single column
  ...
  # Extract the names of the group columns that have a value of 1 and are not 0
  #selected_group_cols <- group_cols[group_col_values & sapply(json_list[group_cols], function(x) x != 0)]
  

  # Extract the names of the group columns that have a value of 1
  #selected_group_cols <- group_cols[which(group_col_values)]
  
  # Remove the group columns that have a value of 0
  #selected_group_cols <- selected_group_cols[colSums(json_list[selected_group_cols]) > 0]
  


  # if (length(selected_group_cols) >=1){
  # modified_json_list[["LTCs2Dimension"]] <- unname(selected_group_cols)
  # }
...
# Test the function
#json_string <- '{"AgeDimension":[[40,44]],"AgeDimension.1":[[0,4]],"AgeDimension.2":[[10,14]],"AgeDimension.3":[[15,19]],"AgeDimension.4":[[20,24]],"AgeDimension.5":[[30,34]],"AgeDimension.6":[[55,59]],"AgeDimension.7":[[65,69]],"AgeDimension.8":[[70,74]],"AgeDimension.9":[[75,79]],"AgeDimension.10":[[80,84]],"AgeDimension.11":[[85,89]],"RskDimension":[[6,120]]}'
#new_json_string <- consolidate_dimensions(json_string)
#print(new_json_string)
...
#   return(modified_json_str)
# }
#print(names(fromJSON(filtered_json_str)))
...
#modified_json_str <- modify_json_zero(filtered_json_str, group_cols)
}
else {
  ...
  observeEvent(input$bngo, {
    if(!is.null(input$bnVar1)) {
      ...
      try({
        ...
        #bayesian_output_df <- convert_bayesian_output_to_df(paste0(fittedbn), colnames(data))
        #print(bayesian_output_df)
